...
def test_backpack():
    ...
    assert 60 == Backpack.best_value_with_unlimited_items(capacity=4, weights=[3, 1, 4], values=[20, 15, 30], iterate_weights_first=True)
    assert 60 == Backpack.best_value_with_unlimited_items(capacity=4, weights=[3, 1, 4], values=[20, 15, 30], iterate_weights_first=False)
