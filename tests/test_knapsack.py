...
def test_knapsack_non_reusable():
    capacity, weights, values = 4, [3, 1, 4], [20, 15, 30]

    # Non reusable, Not fill to capacity
    benchmark_dp_table = [
        [0,  0,  0, 20, 20],
        [0, 15, 15, 20, 35],
        [0, 15, 15, 20, 35]
    ...
    dp_table, item_list = Knapsack.best_value_with_non_reusable_items_2d(
    ...
    assert equal_list(benchmark_dp_table, dp_table) and equal_list(benchmark_item_list, item_list)
    dp_table, item_list = Knapsack.best_value_with_non_reusable_items_1d(
        capacity=capacity, weights=weights, values=values, min_max_function=max,
...
def test_knapsack_reusable():
    capacity, weights, values = 4, [3, 1, 4], [20, 15, 30]
    
    # Reusable, Not fill to capacity
    benchmark_dp_table = [
        [ 0,  0,  0, 20, 20],
        [ 0, 15, 30, 45, 60],
        [ 0, 15, 30, 45, 60],
    ...
    dp_table, item_list = Knapsack.best_value_with_reusable_items_2d(
        capacity=capacity, weights=weights, values=values, min_max_function=max,
        fill_to_capacity=False, iterate_weights_first=True, output_dp_table=True, output_item_list=True
    )
    assert equal_list(benchmark_dp_table, dp_table) and equal_list(benchmark_item_list, item_list)
    ...
    assert equal_list(benchmark_dp_table, dp_table) and equal_list(benchmark_item_list, item_list)
    dp_table, item_list = Knapsack.best_value_with_reusable_items_1d(
        capacity=capacity, weights=weights, values=values, min_max_function=max,
        fill_to_capacity=False, iterate_weights_first=True, output_dp_table=True, output_item_list=True
    )
    assert equal_list(benchmark_dp_table[-1], dp_table) and equal_list(benchmark_item_list[-1], item_list)
